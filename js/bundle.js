...
function calc() {
  ...
  let sex, height, weight, age, ratio; //Сохраняем данные в localStorage и устанавливаем дефолтные значения 
  ...
  initLocalSettings('.calculating__choose_big div', 'calculating__choose-item_active'); // Функция расчета проверяет есть ли данные для расчета и считает по формулам из интернетика
  ...
  calcTotal(); //Ф-я получает данные от юзера 
...
function slider({
  ...
  let slideIndex = 1,
      offset = 0; //                 Проверяем для индексов  01-09 
  ...
  slidesField.style.transition = '0.5s all'; //Скрыли выпадающие слайды из контейнера 

  slidesWrapper.style.overflow = 'hidden'; //не понял зачем перебрал 
...
`;
  ...
  next.addEventListener('click', () => {
    ...
    dotToggler(); // dots.forEach(dot => dot.style.opacity ='.5');
    // dots[slideIndex -1].style.opacity =1;
  });
  ...
  dots.forEach(dot => {
    dot.addEventListener('click', e => {
      ...
      slidesField.style.transform = `translateX(-${offset}px)`; // dots.forEach(dot => dot.style.opacity ='.5');
      // dots[slideIndex -1].style.opacity = 1;
...
__webpack_require__(/*! es6-promise */ "./node_modules/es6-promise/dist/es6-promise.js").polyfill(); // починил слайдер в m.edge 
...
window.addEventListener('DOMContentLoaded', () => {
  const modalTimer = setTimeout(() => (0,_modules_modals__WEBPACK_IMPORTED_MODULE_2__.openModal)('.modal', modalTimer), 50000);
  (0,_modules_tabs__WEBPACK_IMPORTED_MODULE_1__.default)('.tabheader__item', '.tabcontent', '.tabheader__items', 'tabheader__item_active');
  (0,_modules_modals__WEBPACK_IMPORTED_MODULE_2__.default)('[data-modal]', '.modal', modalTimer);
  (0,_modules_timer__WEBPACK_IMPORTED_MODULE_3__.default)('.timer', '2021-05-01');
  (0,_modules_cards__WEBPACK_IMPORTED_MODULE_4__.default)();
  (0,_modules_calc__WEBPACK_IMPORTED_MODULE_5__.default)();
  (0,_modules_forms__WEBPACK_IMPORTED_MODULE_6__.default)('form', modalTimer);
